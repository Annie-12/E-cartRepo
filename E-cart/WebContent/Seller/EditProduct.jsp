...
<%
	if(request.getParameter("sb3")!=null){
		...
		try{
			...
			String sql = "UPDATE product SET status = '"+state+"' WHERE id = '"+prod+"'";
		    int editState=stmt.executeUpdate(sql);
		    if(editState>0){
		    	out.write("<script type='text/javascript'>alert('Updated your product')</script>");
		    	
		    }
		    else{
		    	out.write("<script type='text/javascript'>alert('Could not updated your product')</script>");
		    }
